...
- 新功能优先使用带时区的 UTC 时间。
- 当前请求链使用 SQLAlchemy 异步访问：Router、Service 和数据库依赖保持 `async/await` 一致，不把同步 Session 混入 AsyncSession 调用链。
- CPU 密集或同步阻塞操作不能直接放在事件循环中；短任务使用工作线程，重 CPU 任务使用进程池或独立任务系统。
- 分页总数查询必须与当前筛选条件一致。
