...
当前默认是低成本单机方案：ECS 上的 Docker Compose 同时运行 FastAPI、Nginx、PostgreSQL 和 Redis；
PostgreSQL/Redis 数据目录挂载到 ECS，并由你自行备份。本目录下面的分阶段 Runbook 继续提供备案、
监控和回滚细节，但涉及 RDS/Tair 的段落均属于可选历史方案。
...
见 [阿里云部署错误排查](../故障排查/README.md) 和 [公网 IP 与域名运行文档](../故障排查/02-公网IP与域名运行文档.md)。
逐条学习附件命令的检查顺序和副作用，见 [错误指令学习与排查流程](../故障排查/03-错误指令学习与排查流程.md)。
...
浏览器
  ...
  -> FastAPI 容器（只在 Compose 内网暴露 8000）
       |-> PostgreSQL 容器（Compose 内网 5432）
       |-> Redis 容器（Compose 内网 6379）
...
| PostgreSQL | ECS Docker `postgres` | 数据目录由 `PG_DATA_PATH` 持久化，备份由部署者负责 |
| Refresh Session | ECS Docker `redis` | AOF 持久化到 `REDIS_DATA_PATH`，会话丢失后需重新登录 |
...
如果重点是“如何在 ECS Docker 中启动 PostgreSQL/Redis”，请先读[《Docker 本机 PostgreSQL 与 Redis 正式部署教程》](../运维教程/08-Docker本机PostgreSQL与Redis正式部署.md)。
只有未来购买云托管数据库时，才参考[《RDS 与 Tair 方案说明（历史参考）》](../运维教程/08-RDS与Tair正式生产部署.md)。

## 3. 从采购到上线的总链路

```text
确定主体、地域和域名
  -> 购买域名、ECS、公网带宽/地址和数据盘
  -> 域名实名认证，准备大陆 ICP 备案
  -> 建立 ECS 安全组、数据盘、Docker backend 网络和备份策略
...
| 网络 | Compose 内网可访问 postgres/redis，公网没有开放 5432/6379/8000 |
| 打包 | 测试/静态检查通过，迁移已审查，版本有明确 tag/commit |
| 配置 | Settings 校验通过，Secret/证书不在 Git 或镜像中 |
| 迁移 | PostgreSQL 备份完成，目标数据目录和待执行 Revision 已二次确认 |
...
<PG_DATA_PATH>        PostgreSQL 宿主机数据目录
<REDIS_DATA_PATH>     Redis 宿主机 AOF 数据目录
...
- `DROP DATABASE/TABLE`、清空 PostgreSQL 数据目录、删除数据盘或快照。
...
1. `app/templates/layout.html` 页脚已展示 ICP 备案号；公安联网备案编号取得后仍需按要求补充。
...
5. 当前 Compose 要求启动前已经有 `fullchain.pem` 和 `privkey.key`，没有内置 Certbot 首次
