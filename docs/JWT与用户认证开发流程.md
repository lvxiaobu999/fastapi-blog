...
本文是用户认证的开发方案，不代表功能已经实现。当前项目只有用户 CRUD 和 Argon2 密码哈希，所有 `/api/users` 接口仍未受认证保护。
...
以下文件是实施阶段的建议结构，目前尚未创建：

| 文件 | 计划职责 |
| --- | --- |
| `app/schemas/auth.py` | 定义 `TokenResponse`、可选的 Token Payload 数据契约 |
| `app/services/auth.py` | 验证密码、查找登录用户、创建和解码 JWT |
| `app/dependencies/auth.py` | 定义 OAuth2 Bearer、`get_current_user`、`require_admin` |
| `app/routers/api_auth.py` | 提供 `POST /api/auth/token` 登录接口 |
| `tests/test_auth.py` | 覆盖登录、Token 校验、过期、伪造和权限测试 |
...
POST /api/auth/token
  ...
  → auth_service.authenticate_user()
      → 按 username 查询 User
      → password_hash.verify(明文密码, hashed_password)
